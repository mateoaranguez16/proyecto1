...
<head>
    ...
    <link rel="stylesheet" href="src/pages/css/pago.css" />
    <style>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const envioCosto = parseFloat(localStorage.getItem("envioCosto")) || 0;
  ...
  formTarjeta.addEventListener('submit', async (e) => {
    ...
    const { paymentMethod, error } = await stripe.createPaymentMethod({
      type: 'card',
      card: cardElement,
      billing_details: { name: document.getElementById('titular').value },
    });
    if (error) {
